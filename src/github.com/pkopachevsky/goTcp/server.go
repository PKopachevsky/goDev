...
func (ps *proxyServer) handleClient(conn net.Conn) {
	...
	resp := &dumper{w: conn, label: responseLabel(conn), dumpTo: ps.dumpTo}
...
func responseLabel(conn net.Conn) string{
	return fmt.Sprintf("[==>>] Response from %s", conn.RemoteAddr().String())
}
